...
---
## 🔵 code-agent | Статус: ⏳ В ожидании

### Задача 1: Аудит вебхуков
**Контекст:** Нужно проанализировать систему взаимодействия с бэкендом/вебхуками.
**Задание:**
1. Найти все места в проекте (например, `ai_generation_service.dart`, `game_session.dart` и др.), где происходит вызов вебхуков.
2. Детально расписать, **в какой именно момент игрового цикла** (lifecycle) вызывается каждый вебхук.
3. Ответить результатами анализа **напрямую в чат** пользователю.
4. Подготовить пошаговый план: как мы будем сначала прогонять эти вебхуки в тестах, а затем тестировать в реальной игре.

### Задача 2: Нативная камера с оверлеем и обрезкой фото
**Контекст:** При фотографировании лиц на iOS/Android сейчас используется костыль (видимо, `image_picker`), открывающий системную камеру/галерею без визуального гайда в реальном времени.
**Задание:**
1. Интегрировать пакет `camera` для открытия живого превью прямо внутри приложения.
2. Поверх превью камеры наложить UI с рамкой/овалом для правильного позиционирования лица человека.
3. При захвате кадра (фотографировании) - обрезать итоговую фотографию строго по заданному контуру.
4. Все пиксели, находящиеся за пределами рамки/контура (по бокам), заменить на белый фон (или сделать полностью прозрачными, в зависимости от того, что лучше подходит для отправки).
5. Убедиться, что функционал работает как на Android, так и на iOS.
