...
# Replace group name
dart_code = dart_code.replace('"groupName": "Образование и культура"', '"groupName": "Образование"')

# Replace locations for this group
group_pattern = r'(\{\s*"groupName": "Образование",\s*"locations": \[)(.*?)(\],\s*\})'
new_loc_list = "\n" + "".join([f'        "{l["name"]}",\n' for l in locations]) + "      "
...
for l in locations:
    ...
    if re.search(role_pattern, dart_code, flags=re.DOTALL):
        ...
        # insert at the end of Образование section
        insert_marker = r'(// Образование\s*)'
...
print(f"Updated Group 5 with {len(locations)} locations.")
