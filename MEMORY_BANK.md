...
## 📈 Журнал последних изменений (31 мая 2026)

### 🛡️ Оптимизация верстки, адаптивность и стабильность:
1. **Экран раунда (`game_round_screen.dart`):**
   - Реализована концепция зарезервированного адаптивного контейнера для подсказок (`75-90` пикселей). Это полностью избавило экран от сдвигов элементов («прыжков») при открытии подсказки.
   - Дефолтный шрифт имен уменьшен до `20` (с динамическим сжатием до `18` на мелких экранах) для защиты от длинных имен.
   - Сжаты стрелочка хода (`30-40`) и паддинги Turn-блока.
   - Скрыт таймер овертайма при мигании надписи «ВРЕМЯ ВЫШЛО!».
2. **Настройки тарифов (`settings_screen.dart`):**
   - Премиум-диалог выбора тарифов переписан с адаптивными размерами под мелкие экраны (`screenHeight < 750`) и обернут в `SingleChildScrollView` (100% защита от `RenderFlex overflow`).
3. **Безопасность игрового потока (`pre_game_flow_screen.dart` & `round_score_screen.dart`):**
   - Убрали `ExitGameButton` (крестик выхода) с экрана очков раунда и с этапа раскрытия карт (`cardReveal`) во избежание случайных закрытий.
   - Исправили цвет кнопки «Фото история игры» на золотой акцентный (`AppStyles.accent`).
   - Починили передачу локации (`locationName`) в карточках истории раундов.
4. **Конвейер сборки (`scripts/upload_apk.py` & `memory_bank/agent_instructions.md`):**
   - Написали автоматический скрипт загрузки собранных Flutter APK на Google Drive с автоименованием (`spy_X.apk`).
   - Добавили пошаговую инструкцию в базу знаний AI-агентов по команде «собери апк».
