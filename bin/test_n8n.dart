...
void main() async {
  ...
  const password =
      'your_real_password_here'; // Intentionally placeholder to not leak in script, but it doesn't matter for a basic connection test if we just want to see if the server responds at all (even with 401)
  ...
  try {
    final response = await http
        .get(Uri.parse('https://n8n.sharksbots.com/'))
  ...
  try {
    ...
    print(
      'Body (first 100 chars): ${response.body.length > 100 ? response.body.substring(0, 100) : response.body}',
    );
  } catch (e) {
